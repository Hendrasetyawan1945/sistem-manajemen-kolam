<x-layouts.admin>
    <x-page-header 
        ...
        subtitle="Perbarui data pengeluaran"
        :breadcrumbs="[
                        ...
                        <div class="mb-3">
                            ...
                            <select name="item_kas_id" id="item_kas_id" class="form-control @error('item_kas_id') is-invalid @enderror">
                                ...
                                @foreach($itemKasList as $item)
                                    <option value="{{ $item->id }}" {{ old('item_kas_id', $pengeluaran->item_kas_id) == $item->id ? 'selected' : '' }}>
                                        {{ $item->nama }}
                                    </option>
                                @endforeach
                            </select>
                ...
                <div class="row">
                    <div class="col-md-12">
                        <div class="mb-3">
                            ...
                            <textarea name="keterangan" id="keterangan" rows="4" class="form-control @error('keterangan') is-invalid @enderror" placeholder="Keterangan pengeluaran (opsional)">{{ old('keterangan', $pengeluaran->keterangan) }}</textarea>
                            @error('keterangan')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
