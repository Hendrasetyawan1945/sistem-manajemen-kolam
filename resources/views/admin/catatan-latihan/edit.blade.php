<x-layouts.admin>
    <x-page-header 
        ...
        subtitle="Perbarui catatan waktu latihan"
        :breadcrumbs="[
    ...
    <div class="card shadow mb-4">
        ...
        <div class="card-body">
            <form action="{{ route('admin.catatan-latihan.update', $catatanLatihan) }}" method="POST">
                @csrf
                @method('PUT')
                        ...
                        <div class="mb-3">
                            ...
                            <select name="gaya_renang" id="gaya_renang" class="form-control @error('gaya_renang') is-invalid @enderror" required>
                                ...
                                @foreach($gayaRenangList as $gaya)
                                    <option value="{{ $gaya }}" {{ old('gaya_renang', $catatanLatihan->gaya_renang) == $gaya ? 'selected' : '' }}>
                                        {{ $gaya }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-3">
                            ...
                            <select name="jarak" id="jarak" class="form-control @error('jarak') is-invalid @enderror" required>
                                ...
                                @foreach($jarakList as $j)
                                    <option value="{{ $j }}" {{ old('jarak', $catatanLatihan->jarak) == $j ? 'selected' : '' }}>
                                        {{ $j }}m
                                    </option>
                                @endforeach
                            </select>
                ...
                <div class="row">
                    <div class="col-md-12">
                        <div class="mb-3">
                            ...
                            <textarea name="catatan" id="catatan" rows="3" class="form-control @error('catatan') is-invalid @enderror" placeholder="Catatan tambahan (opsional)">{{ old('catatan', $catatanLatihan->catatan) }}</textarea>
                            @error('catatan')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
