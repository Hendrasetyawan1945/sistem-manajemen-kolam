<x-layouts.admin>
    <x-page-header 
        title="Edit Pendaftaran Kejuaraan"
        subtitle="Perbarui data pendaftaran kejuaraan"
        :breadcrumbs="[
    ...
    <div class="card shadow mb-4">
        ...
        <div class="card-body">
            <form action="{{ route('admin.iuran-kejuaraan.update', $iuranKejuaraan) }}" method="POST">
                @csrf
                @method('PUT')
                            ...
                            <select name="kejuaraan_id" id="kejuaraan_id" class="form-control @error('kejuaraan_id') is-invalid @enderror" required>
                                ...
                                @foreach($kejuaraanList as $kejuaraan)
                                    <option value="{{ $kejuaraan->id }}" 
                                        ...
                                        {{ old('kejuaraan_id', $iuranKejuaraan->kejuaraan_id) == $kejuaraan->id ? 'selected' : '' }}>
                        ...
                        <div class="mb-3">
                            ...
                            <select name="siswa_id" id="siswa_id" class="form-control @error('siswa_id') is-invalid @enderror" required>
                                ...
                                @foreach($siswaList as $siswa)
                                    <option value="{{ $siswa->id }}" {{ old('siswa_id', $iuranKejuaraan->siswa_id) == $siswa->id ? 'selected' : '' }}>
                ...
                <div class="row">
                    <div class="col-md-6">
                        <x-form-input 
                            ...
                            name="jumlah"
                            type="number"
                            :value="old('jumlah', $iuranKejuaraan->jumlah)"
                            placeholder="500000"
                            required
                            id="jumlah"
                        />
                    ...
                    <div class="col-md-6">
                        <div class="mb-3">
                            ...
                            <select name="status_bayar" id="status_bayar" class="form-control @error('status_bayar') is-invalid @enderror" required>
                                <option value="belum" {{ old('status_bayar', $iuranKejuaraan->status_bayar) == 'belum' ? 'selected' : '' }}>Belum Bayar</option>
                                <option value="lunas" {{ old('status_bayar', $iuranKejuaraan->status_bayar) == 'lunas' ? 'selected' : '' }}>Lunas</option>
                            </select>
                            @error('status_bayar')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            <x-form-input 
                                ...
                                name="tanggal_bayar"
                                type="date"
                                :value="old('tanggal_bayar', $iuranKejuaraan->tanggal_bayar?->format('Y-m-d'))"
                                id="tanggal_bayar"
                            />
                        </div>
                            ...
                            <div class="mb-3">
                                ...
                                <select name="metode_pembayaran_id" id="metode_pembayaran_id" class="form-control @error('metode_pembayaran_id') is-invalid @enderror">
                                    ...
                                    @foreach($metodePembayaran as $metode)
                                        <option value="{{ $metode->id }}" {{ old('metode_pembayaran_id', $iuranKejuaraan->metode_pembayaran_id) == $metode->id ? 'selected' : '' }}>
                                            {{ $metode->nama }}
                                        </option>
                                    @endforeach
                                </select>
